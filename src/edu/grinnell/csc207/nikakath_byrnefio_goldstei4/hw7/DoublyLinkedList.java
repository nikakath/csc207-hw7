...
public class DoublyLinkedList<T> implements ListOf<T> {
	...
	public void insert(T val, Cursor c) throws Exception {
		...
		if (this.front == null) {
			/*
			 * this.front = in; this.back = in;
			 */
			this.prepend(val);
		} else if (curs.pos == this.front) {
			/*
			 * in.next = this.front.next; this.front.prev = in; this.front = in;
			 */
			this.prepend(val);
		...
		/*
		 * in.prev = this.back.prev; this.back.next = in; this.back = in;
		 */
		// this.append(val);
	...
	/**
	 * Get a cursor right before the front of the list.
	 * 
	 * @throws Exception
